// Framework-free TRACKiT HTTP core: pacing, retry, auth, and the endpoint
// wrappers. `@/lib/trackit/client` re-exports all of this behind a
// `server-only` guard for use inside Next; standalone scripts (e.g. the
// one-off backfill) import this module directly so the guard doesn't trip.
...
// Every account this codebase knows how to talk to, in priority order. Read
// fresh each call (not a module-level const) so a script that loads its .env
// after importing this module still sees the credentials.
function accountEnv(): Array<{ id: string; user?: string; pass?: string }> {
  return [
 ...
 * third account is just another accountEnv() row.
 */
export function getConfiguredAccounts(): TrackitAccount[] {
  return accountEnv().filter(
...
// One raw position record from GET-history (`/vehicleHistoric`). Confirmed via
// real payload: `tmx` (ISO ts), `vid`, `loc.{lat,lon}`, `gsp` (km/h), `gkm`
// (odometer km — reads 0 when the vehicle is off/stationary, real value only
// while running). No plate and no POI match in this payload.
export type TrackitHistoricPoint = {
  vid: number;
  tmx?: string | null;
  loc?: { lat?: number | null; lon?: number | null } | null;
  gsp?: number | null;
  gkm?: number | null;
  [key: string]: unknown;
};

export type ParsedHistoricPoint = {
  vehicleId: number;
  latitude: number | null;
  longitude: number | null;
  speedKmh: number | null;
...
export function parseHistoricPoint(p: TrackitHistoricPoint): ParsedHistoricPoint {
  const loc = (p.loc ?? {}) as { lat?: unknown; lon?: unknown };
  const gkm = typeof p.gkm === "number" ? p.gkm : null;
  return {
    vehicleId: typeof p.vid === "number" ? p.vid : Number(p.vid),
    latitude: typeof loc.lat === "number" ? loc.lat : null,
    longitude: typeof loc.lon === "number" ? loc.lon : null,
    speedKmh: typeof p.gsp === "number" ? p.gsp : null,
    // 0 = "no reading" for a parked vehicle; never store it as a real odometer.
    odometerKm: gkm && gkm > 0 ? gkm : null,
    recordedAt: normalizeTrackitTimestamp(p.tmx),
  };
}

// Raw position history for one vehicle over a time range. `dateBegin`/`dateEnd`
// are "YYYY-MM-DD HH:MM:SS" in UTC, same as getVehicleTravels. Not used by the
// live sync (which reads current positions off /vehiclesForUser) — this is for
// backfilling vehicle_pings from history.
export function getVehicleHistoric(
  account: TrackitAccount,
  vehicleId: number,
  dateBegin: string,
  dateEnd: string,
): Promise<TrackitHistoricPoint[]> {
  return callTrackit<TrackitHistoricPoint[]>(account, "/vehicleHistoric", {
    method: "POST",
    body: JSON.stringify({ vehicleId, dateBegin, dateEnd }),
  });
}
